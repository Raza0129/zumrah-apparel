...
import type { ProductDetailData, ProductColor } from "@/lib/types";
...
const QUICK_PRESETS = ["BRAND NAME", "Team 2026", "Custom Quote", "LOGO TEXT", "Event Name", "Player #10"];
const QUICK_EMOJI = ["⭐", "🔥", "🏆", "💎", "🦁", "⚡", "🎯", "🌟"];

const TEMPLATES: { name: string; layers: Partial<Layer>[] }[] = [
  { name: "Minimal Logo", layers: [{ type: "text", text: "BRAND", fontSize: 32, fontColor: "#ffffff", bold: true, fontFamily: "Impact" }] },
  { name: "Sports Team", layers: [{ type: "text", text: "TEAM", fontSize: 36, fontColor: "#D4AF37", bold: true, fontFamily: "Impact" }, { type: "text", text: "#10", fontSize: 48, fontColor: "#ffffff", bold: true, fontFamily: "Poppins" }] },
  { name: "Corporate", layers: [{ type: "text", text: "Company Name", fontSize: 22, fontColor: "#ffffff", bold: false, fontFamily: "Inter" }] },
  { name: "Event", layers: [{ type: "text", text: "Event 2026", fontSize: 28, fontColor: "#D4AF37", bold: true, fontFamily: "Poppins" }] },
];
...
// ─── Shirt SVG ──────────────────────────────────────────────────────────────────
function ShirtSVG({ color, view }: { color: string; view: ViewSide }) {
  return (
...
function LayerElement({
  ...
  onRotateMouseDown: (e: React.MouseEvent, layerId: string) => void;
  onResizeMouseDown: (e: React.MouseEvent, layerId: string, handle: string) => void;
}) {
  return (
    <div
    ...
    >
      ...
      {isSelected && !isMulti && !layer.locked && (
        <>
          <div
            onMouseDown={(e) => { e.stopPropagation(); onRotateMouseDown(e, layer.id); }}
            style={{
          ...
          {RESIZE_HANDLES.map((h) => (
            <div
              key={h.id}
              onMouseDown={(e) => { e.stopPropagation(); onResizeMouseDown(e, layer.id, h.id); }}
              style={{
                position: "absolute",
                ...h.style,
                width: 10, height: 10,
...
// ─── Main Component ────────────────────────────────────────────────────────────
export function DesignerClient({ product }: { product: ProductDetailData }) {
  const router = useRouter();
  const { data: session } = useSession();
  const addItem = useCartStore((s) => s.addItem);

  const defaultColor: ProductColor = product.colors[0] ?? { name: "Black", hex: "#111111" };
  ...
  useEffect(() => {
    const onMove = (e: MouseEvent) => {
      ...
      if (drag.mode === "move") {
        ...
        if (snapEnabled) newRot = Math.round(newRot / 15) * 15;
    ...
    const handler = (e: KeyboardEvent) => {
      ...
      if (e.ctrlKey || e.metaKey) {
        switch (e.key.toLowerCase()) {
          case "z": if (e.shiftKey) redo(); else undo(); e.preventDefault(); break;
          ...
          case "g": if (e.shiftKey) ungroupSelected(); else groupSelected(); e.preventDefault(); break;
        }
      } else {
  ...
  // ── Export ──
  const exportSideToPNG = useCallback(async (side: ViewSide, transparent = false): Promise<string> => {
    const SCALE = 3;
    ...
    for (const layer of sideLayers) {
      ...
      if (layer.type === "text") {
        ...
        try {
          ...
          // skip images that fail to load (e.g. cross-origin)
        }
      }
      ctx.restore();
    }
  ...
  const handleSaveDesign = async () => {
    ...
    try {
      ...
      setSavedDesignId(res.designId);
      setSavedPreviewUrl(frontPreviewUrl);
      ...
      <div className="flex flex-1 overflow-hidden min-h-0">
        ...
        <div className="w-56 bg-[#0d0d0d] border-r border-[#1a1a1a] flex flex-col flex-shrink-0 overflow-hidden">
          ...
          <div className="flex-1 overflow-y-auto p-2.5 space-y-3 min-h-0">
            <div>
              <p className="text-gray-600 text-[9px] uppercase tracking-wider mb-1.5">Product Color</p>
              <div className="flex flex-wrap gap-1.5">
                {product.colors.map((c) => (
                  <button key={c.hex} title={c.name} onClick={() => setSelectedColor(c)}
                    className={`w-6 h-6 rounded-full border-2 transition-all hover:scale-110 ${selectedColor.hex === c.hex ? "border-[#D4AF37] scale-110 shadow-[0_0_6px_rgba(212,175,55,0.7)]" : "border-[#333]"}`}
            ...
            <div>
              <p className="text-gray-600 text-[9px] uppercase tracking-wider mb-1.5">Size</p>
              <div className="flex flex-wrap gap-1.5">
                {product.sizes.map((size) => (
                  <button key={size} onClick={() => setSelectedSize(size)}
                    className={`px-2.5 py-1 rounded-lg text-[10px] font-medium transition-all border ${selectedSize === size ? "bg-[#D4AF37] text-black border-[#D4AF37]" : "border-[#333] text-gray-400 hover:border-[#555] hover:text-gray-200"}`}
                  >
                    {size}
                  </button>
                ))}
                    ...
                    onClick={() => {
                      ...
                      tmpl.layers.forEach((l) => {
                        addLayer({
                          ...
                          name: l.text ?? "Layer",
                          side: activeView,
                          x: 30, y: yOffset, width: 150, height: 50, rotation: 0, opacity: 1,
                          visible: true, locked: false,
                          text: l.text, fontSize: l.fontSize, fontColor: l.fontColor,
                          bold: l.bold, fontFamily: l.fontFamily,
                        });
                        yOffset += 60;
                      });
                    }}
        ...
        <div ref={containerRef} className="flex-1 bg-[#080808] flex items-center justify-center overflow-hidden relative">
          ...
          >
            <div className="absolute inset-0">
              <ShirtSVG color={selectedColor.hex} view={activeView} />
            </div>
            ...
            >
              {currentViewLayers.map((layer) => (
                <LayerElement
                  ...
                  isMulti={selectedIds.length > 1 && selectedIds.includes(layer.id)}
                    ...
                    {([
                      ...
                      <div key={label}>
                        ...
                        <input
                          ...
                          onChange={(e) => updateLayer(selectedLayer.id, { [key]: Number(e.target.value) } as Partial<Layer>)}
                        ...
                        {([
                          ...
                          return (
                            <button key={key} title={tip}
                              onClick={() => updateLayer(selectedLayer.id, { [key]: !active } as Partial<Layer>)}
                              className={`flex-1 p-1.5 rounded-lg border transition-all ${active ? "bg-[#D4AF37]/20 border-[#D4AF37]/60 text-[#D4AF37]" : "border-[#333] text-gray-400 hover:text-gray-200"}`}>
              ...
              <div className="bg-[#111] border border-[#1e1e1e] rounded-xl p-3">
                ...
                <div className="flex gap-1 mt-1.5">
                  {product.colors.slice(0, 6).map((c) => (
                    <button key={c.hex} title={c.name} onClick={() => setSelectedColor(c)}
                      className={`w-4 h-4 rounded-full border transition-all ${selectedColor.hex === c.hex ? "border-[#D4AF37] scale-110" : "border-[#333]"}`}
                      style={{ backgroundColor: c.hex }}
                    />
                  ))}
                </div>
        ...
        {addedOverlay && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="absolute inset-0 bg-black/60 flex items-center justify-center z-50">
            <motion.div initial={{ scale: 0.5 }} animate={{ scale: 1 }} className="bg-[#111] border border-[#D4AF37]/40 rounded-3xl p-8 text-center">
              ...
              <h3 className="text-white text-xl font-bold mb-2" style={{ fontFamily: "Poppins, sans-serif" }}>Added to Cart!</h3>
